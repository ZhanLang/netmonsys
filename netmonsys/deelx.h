...
public:
	...
	{
		return ExAllocatePoolWithTag(NonPagedPool, size, 'DEel');
	}
	...
	void SetMaxLength(int nSize)
	{
		...
		{
			int tempSize = m_nMaxLength;
			ELT * tempPoint = CBufferRefT <ELT> ::m_pBuffer;
			if( m_nMaxLength < 8 )
				m_nMaxLength = 8;
			...
			memcpy(CBufferRefT <ELT> ::m_pBuffer, tempPoint, sizeof(ELT) * tempSize);
			free(tempPoint);
		}
	}
...
template <class ELT> void CBufferT <ELT> :: Append(const ELT * pcsz, int length, int eol)
{
	...
	if(nNewLength > m_nMaxLength)
	{
		...
		void* tempPoint = CBufferRefT <ELT> ::m_pBuffer;
		CBufferRefT <ELT> ::m_pBuffer = (ELT *)malloc(sizeof(ELT) * nNewLength);
		memcpy(CBufferRefT <ELT> ::m_pBuffer, tempPoint, sizeof(ELT) * m_nMaxLength);
		free(tempPoint);
	...
	case RCHART('{'):
		{
			...
			{
				re.Append(((curr.ch & (CHART)0xff) == curr.ch) ? (CHART)curr.ch : (CHART)0, (CHART)1);
				MoveNext();
			}
	...
	case RCHART('['):
		{
			...
			if( flags & IGNORECASE )
			{
				...
				for(i=0; i<oldcount; i++)
				{
					...
					{
						newmin = (CHART)tolower( max(RCHART('A'), ranges[i*2  ]) );
						newmax = (CHART)tolower( min(RCHART('Z'), ranges[i*2+1]) );
					...
					if( ranges[i*2] <= RCHART('z') && ranges[i*2+1] >= RCHART('a') )
					{
						newmin = (CHART)toupper( max(RCHART('a'), ranges[i*2  ]) );
						newmax = (CHART)toupper( min(RCHART('z'), ranges[i*2+1]) );
				...
				for(i=0; i<oldcount; i++)
				{
					if( isupper(chars[i]) && ! pRange->IsContainChar((CHART)tolower(chars[i])) )
						chars.Push((CHART)tolower(chars[i]));

					if( islower(chars[i]) && ! pRange->IsContainChar((CHART)toupper(chars[i])) )
						chars.Push((CHART)toupper(chars[i]));
				}
			}
	...
	if(curr == CHART_INFO(RCHART('?'), 1))
	{
		...
		case RCHART('R'):
			...
			if(curr.ch == RCHART('<') || curr.ch == RCHART('\''))
			{
				...
				{
					name.Append(curr.ch, 1);
					num .Append(((curr.ch & (CHART)0xff) == curr.ch) ? (CHART)curr.ch : (CHART)0, (CHART)1);
					MoveNext();
				}
				MoveNext(); // skip '>' or '\''
